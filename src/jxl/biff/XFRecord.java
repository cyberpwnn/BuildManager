...
public class XFRecord extends WritableRecordData implements CellFormat
{
	...
	{ 0xe, 0xf, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x2d, 0x2e, 0x2f };

	/**
	 * The list of java date format equivalents
	 */
	private static final DateFormat[] javaDateFormats = new DateFormat[]
	{ SimpleDateFormat.getDateInstance(DateFormat.SHORT), SimpleDateFormat.getDateInstance(DateFormat.MEDIUM), new SimpleDateFormat("d-MMM"), new SimpleDateFormat("MMM-yy"), new SimpleDateFormat("h:mm a"), new SimpleDateFormat("h:mm:ss a"), new SimpleDateFormat("H:mm"), new SimpleDateFormat("H:mm:ss"), new SimpleDateFormat("M/d/yy H:mm"), new SimpleDateFormat("mm:ss"), new SimpleDateFormat("H:mm:ss"), new SimpleDateFormat("mm:ss.S") };

	/**
	 * The list of built in number format values
	 */
	private static int[] numberFormats = new int[]
	{ 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x30 };

	/**
	 * The list of java number format equivalents
	 */
	private static NumberFormat[] javaNumberFormats = new NumberFormat[]
	{ new DecimalFormat("0"), new DecimalFormat("0.00"), new DecimalFormat("#,##0"), new DecimalFormat("#,##0.00"), new DecimalFormat("$#,##0;($#,##0)"), new DecimalFormat("$#,##0;($#,##0)"), new DecimalFormat("$#,##0.00;($#,##0.00)"), new DecimalFormat("$#,##0.00;($#,##0.00)"), new DecimalFormat("0%"), new DecimalFormat("0.00%"), new DecimalFormat("0.00E00"), new DecimalFormat("#,##0;(#,##0)"), new DecimalFormat("#,##0;(#,##0)"), new DecimalFormat("#,##0.00;(#,##0.00)"), new DecimalFormat("#,##0.00;(#,##0.00)"), new DecimalFormat("#,##0;(#,##0)"), new DecimalFormat("$#,##0;($#,##0)"), new DecimalFormat("#,##0.00;(#,##0.00)"), new DecimalFormat("$#,##0.00;($#,##0.00)"), new DecimalFormat("##0.0E0") };
	...
	public XFRecord(FontRecord fnt, DisplayFormat form)
	{
		...
		indentation = 0;
		shrinkToFit = false;
		usedAttributes = (byte) (USE_FONT | USE_FORMAT | USE_BACKGROUND | USE_ALIGNMENT | USE_BORDER);

		// This will be set by the initialize method and the subclass
		// respectively
		parentFormat = 0;
		xfFormatType = null;
	...
	protected XFRecord(CellFormat cellFormat)
	{
		...
		// The font or format indexes need to be set, so set initialized to
		// false
		initialized = false;
	}
	 ...
	 *
	 * @param pos
	 *            the xf index to initialize this record with
	 * @param fr
	 *            the containing formatting records
	 * @param fonts
	 *            the container for the fonts
	...
	public int hashCode()
	{
		// Must have its formats info initialized in order to compute the hash
		// code
		if(!formatInfoInitialized)
		{
			initializeFormatInformation();
	...
	public boolean equals(Object o)
	{
		...
		if(align != xfr.align || valign != xfr.valign || orientation != xfr.orientation || wrap != xfr.wrap || shrinkToFit != xfr.shrinkToFit || indentation != xfr.indentation)
		{
			return false;
		}
		...
		else
		{
			...
			if(!font.equals(xfr.font) || !format.equals(xfr.format))
			{
				return false;
			}
		}
	...
	public void setFont(FontRecord f)
	{
		...
		// However, sometimes (when setting a row or column format) an
		// initialized
